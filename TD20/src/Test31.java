...
//	private final static int source = 190637;
	...
	public static void main(String args[]) throws Exception {
		...
		// Le test de getMinPath() suppose que compute() est correct
	...
	static void testFlip(Graph g, String name, int source, int dest) throws Exception {
		...
		assert bothways.currentDijkstra != bothways.otherDijkstra :
			"\n\ncurrentDijkstra == otherDijkstra' après un appel à flip()\n";
	...
	static void testCompute(Graph g, String name, int source, int dest, int length) throws Exception {
		...
		assert bothways.isOver() :
			"\n\ncompute() ne va pas jusqu'au bout\n";
		...
		assert (fSettled - bSettled == 0) || (fSettled - bSettled == 1) : 
			"\n\nle nombre de sommets traités par les deux parcours devrait différer d'au plus un\n";

		assert result == bothways.getMinPath() :
			"\n\nrésultat de compute() != longueur du chemin calculée par getMinPath()\n";
	...
	static void testGetMinPath(Graph g, String name, int source, int dest, int naive) throws Exception {
		...
		assert minPath == bothways.forward.getDist()[bothways.getLast()] + bothways.backward.getDist()[bothways.getLast()] :
			"\n\nLa valeur renvoyée par getMinPath() n'est pas la somme de distances à 'last' de 'source' et de 'dest'\n";
		
		System.out.println("[OK]");
		
		if (minPath < naive)
			System.err.println("\nIl semblerait que vous essayiez de tester la version modifiée de la méthode getMinPath()"
